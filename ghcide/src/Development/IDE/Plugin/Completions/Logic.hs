  ...
  let dflags = hsc_dflags packageState
      ...
      toCompItem par m mn n imp' = do
        ...
        let mbParent = case par of
                            NoParent -> Nothing
                            ParentIs n -> Just (showName n)
                            FldParent n _ -> Just (showName n)
...
findRecordCompl uri pmod mn DataDecl {tcdLName, tcdDataDefn} = result
    where
        result = [mkRecordSnippetCompItem uri (Just $ showName $ unLoc tcdLName)
                        (T.pack . showGhc . unLoc $ con_name) field_labels mn doc Nothing
